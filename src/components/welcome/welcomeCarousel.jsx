      ...
      <div className="w-full overflow-hidden">
        ...
        >
          {slides.map((slide, index) => (
            <div key={index} className="flex flex-col md:flex-row items-center justify-center w-full flex-shrink-0">
              {/* Image Section */}
              <div className="w-full md:w-1/2 h-[70vh] sm:h-[80vh] flex items-center justify-center">
                <img src={slide.image} alt="" className="w-full h-auto max-w-full rounded-lg object-contain" />
              </div>

              {/* Text & QR Code Section */}
              <div className="w-full md:w-1/2 py-4 px-6 flex flex-col h-[60vh] sm:h-[70vh] justify-between">
                <div className="text-left">
                  <h2 className="text-lg md:text-2xl lg:text-3xl font-bold text-center">{slide.title}</h2>
      ...
      <div className="absolute bottom-4 left-1/2 md:left-1/3 transform -translate-x-1/2 flex justify-center space-x-2 sm:space-x-3">
        {slides.map((_, index) => (
          <button key={index} onClick={() => setCurrentSlide(index)}
            className={`w-2 h-2 sm:w-3 sm:h-3 md:w-4 md:h-4 rounded-full ${currentSlide === index ? "bg-white" : "bg-gray-400"}`}>
          </button>
        ))}
      </div>
    </div>
